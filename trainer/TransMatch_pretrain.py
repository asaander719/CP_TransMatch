...
class TransMatch(Module):
    def __init__(self, conf, u_topk_IJs, i_topk_UJs, j_topk_UIs, neighbor_params=None, visual_features=None, item_cate=None):        
    ...
    def find_topk_js_for_ui(self, U_latent, I_latent, J_bias, J_latent, all_embs):
        ...
        topk_scores, topk_indices = torch.topk(distances, self.topk_i, dim=-1) #bs, topk
...
#         res = edge_vectors[0].view([bs, self.n_relations])
    ...
    def _aggregate_neighbors_test(self, edge_list, entity_list, mask_list, relation_features, entity_features, visual=False):
        ''' A list of masks, indicating the presence or absence of edges in the context.'''
        ...
        if self.pretrain_mode:
            ...
            if self.context_enhance:
                ...
                if self.score_type == "mlp":
                    ...
                    R_j_c = self.transE_predict(U_latent, I_latent, J_latent, J_bias_l)
                    R_k_c = self.transE_predict(U_latent, I_latent, K_latent, K_bias_l)
                if self.use_context:
                    R_j += R_j_c
                    R_k += R_k_c
                else:
    ...
    def aggregate_item_embeddings(self, item_embeddings, topk_indices, batch_item_embeddings, att_model):
        topk_item_embeddings = item_embeddings(topk_indices)
        aggregated_embeddings = att_model(batch_item_embeddings, topk_item_embeddings)
        return aggregated_embeddings
        
    def aggregate_embeddings(self, user_embeddings, topk_indices, batch_user_embeddings, att_model_u):
    ...
    def _group_topkIJs_for_U_(self, item_embeddings, Us, batch_user_embeddings, att_model_u):
        ...
        combined_tensor = torch.cat((topk_Is_embeddings, topk_Js_embeddings), dim=1) #bs, topk_u*2, hd
        ...
        aggregated_embeddings = batch_user_embeddings + torch.mean(combined_tensor, dim=1) * self.agg_param#.view([batch_user_embeddings.size(1), self.emb_dim])
        return aggregated_embeddings

    def _group_topkIJs_for_U_v_(self, item_embeddings, Us, batch_user_embeddings, att_model_u):
        topk_Is = self.u_topk_IJs[Us] # bs, 2, topk_u
        topk_Is = topk_Is[:, 0, :self.topk_u] # bs, topk_u
        topk_Is_embeddings = item_embeddings[topk_Is] #bs, topk_u, hd # torch.Size([1024, 3, 32])
        topk_Js = self.u_topk_IJs[Us]
        topk_Js = topk_Js[:, 1, :self.topk_u]
        topk_Js_embeddings = item_embeddings[topk_Js] #bs, topk_u, hd
        combined_tensor = torch.cat((topk_Is_embeddings, topk_Js_embeddings), dim=1) #bs, topk_u*2, hd
    ...
    def _group_topkUJs_for_I_(self, item_embeddings, Is, batch_Is_embeddings, att_model_u):
        ...
        topk_Us_embeddings = self.transe.u_embeddings_l(topk_Us) #bs, topk_u, hd
        topk_Js = topk_UJs[:, 1, :self.topk_i]
        topk_Js_embeddings = item_embeddings(topk_Js) #bs, topk_u, hd
        combined_tensor = torch.cat((topk_Us_embeddings, topk_Js_embeddings), dim=1) #bs, topk_u*2, hd
    ...
    def _group_topkUJs_for_I_v_(self, item_embeddings, Is, batch_Is_embeddings, att_model_u):
        topk_UJs = self.i_topk_UJs[Is]
        topk_Us = topk_UJs[:, 0, :self.topk_i] # bs, 2, topk_u -> bs, topk_u
        topk_Us_embeddings = self.transe.u_embeddings_v(topk_Us) #bs, topk_u, hd
        topk_Js = topk_UJs[:, 1, :self.topk_i]
        topk_Js_embeddings = item_embeddings[topk_Js] #bs, topk_u, hd
        combined_tensor = torch.cat((topk_Us_embeddings, topk_Js_embeddings), dim=1) #bs, topk_u*2, hd
        # aggregated_embeddings = att_model_u(batch_Is_embeddings, combined_tensor)
    ...
    def inference(self, batch):
        ...
        if self.pretrain_mode:
            ...
            if self.context_enhance:
                ...
                I_latent = self._group_topkUJs_for_I_(self.transe.i_embeddings_i, Is, I_latent_ori, self.self_attention_u)
                J_latent = self._group_topkUJs_for_I_(self.transe.i_embeddings_i, Js, J_latent_ori, self.self_attention_u)
                K_latent = self._group_topkUJs_for_I_(self.transe.i_embeddings_i, Ks.squeeze(1), K_latent_ori, self.self_attention_u)
                # K_latent = K_latent_ori
                if self.use_Nor:
                    U_latent = F.normalize(U_latent,dim=0)
                    I_latent = F.normalize(I_latent,dim=0)
                ...
                if self.score_type == "mlp":
                    ...
                    scores_c = self.transE_predict(U_latent, I_latent, Js_latent_ii, J_bias_l)
                    if self.use_context:
                        scores += scores_c
                    else:
                        scores = scores_c
            ...
            if self.context_enhance:
                ...
                I_visual = self._group_topkUJs_for_I_v_(all_item_latent_v, Is, I_visual_ori, self.self_attention_v_u)
                J_visual = self._group_topkUJs_for_I_v_(all_item_latent_v, Js, J_visual_ori, self.self_attention_v_u) 
                K_visual = self._group_topkUJs_for_I_v_(all_item_latent_v, Ks.squeeze(1), K_visual_ori, self.self_attention_v_u)
                # K_visual = K_visual_ori
                if self.use_Nor:
                    U_visual = F.normalize(U_visual,dim=0)
                    I_visual = F.normalize(I_visual,dim=0)
